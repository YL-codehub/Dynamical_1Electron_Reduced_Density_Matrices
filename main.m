%% 1-RDM REFINEMENT SOFTWARE UNDER MATLAB
% by Yoann Launay*, initiated by Benjamin De Bruyne*.
% Former CentraleSupélec students at Paris-Saclay University.

%% - Feel free to contact yoann.launay@outlook.com for more explanations
...
% from experimental data sets through semidefinite programming",
%B. De Bruyne and J-M Gillet, Acta cristallographica A, 2020

% - All calculations are in a.u.


%% EACH FUNCTION HAS AN UPDATED DOCUMENTATION (inputs, outputs, goal and methods).
% Just call : help('functionName') in the command window and you'll see.

%% PREREQUISITES BEFORE ANY CALCULUS :
% - You need to either re-install mosek and yalmip completely in this
% repertory or to add them to your path
% - You need to get a Mosek license and put it in your home dir as explained in
% https://docs.mosek.com/9.0/install/installation.html (choose free 1 year
% academic license)
% - For any dilation, you need the Optimization Toolbox which can be
% installed at the installation of Matlab.

%% SOME ISSUES I FACED AND HOW I SOLVED THEM:
% - You also need to add to path every file of these packages (especially
...
% - if you keep the packages already installed by your predecessor, you may will have an "invalid MEX-file" issue
% see then section "troubleshooting" at https://docs.mosek.com/9.2/toolbox/install-interface.html to
% update without install again. For exemple you may need the following command line
% to be uncommented (change the source path):
% setenv('PATH', [getenv('PATH') ';D:\Cristallographie quantique\cleanRDM\RDM_SDP\mosek\8\tools\platform\win64x86\bin']);

%% Add SDP optimisation toolbox to Matlab's path 
import OPTIMLib.*

%% IMPORT coords, basis set

[ coords, R, T,volume, primVectors ] = readCoords('InputFiles/coordsInCell.ent');
% - 'coords': coordinates of three atoms O, C, O
% - 'R': rotation matrix (3x3x4) to rotate 'coords' to the axis of ith molecule in unit
% cell (i=1,...,4)
% - 'T': translation vectors (4x3) to go from 'coords' to the position of ith molecule in unit
% cell (i=1,...,4)
% - 'volume': volume of the primitive cell
% - 'primVectors': primitive vectors

[atomicOrbitals,~] = readBS('InputFiles/BS.d12',coords);
% - 'atomicOrbitals': GTO structure orbitals for all atoms.


%% IN THE FOLLOWING, MANY COMMAND LINES ARE WRITTEN, 
%% UNCOMMENT THEM (ONE SECTION AT A TIME, AND ONLY LINES WITH A SINGLE %)
%% TO UNDERSTAND HOW IT WORKS

%% EXAMPLE 1 : Import expectation values and add noise

% % Different files are needed to read data on (see readCrystal.m's doc).
% % An example at 0K (see readCrystal.m's doc for more details): SF data in the Ewald sphere of radius 8 and DCPs under 6 u.a momentum with a
% % resolution of 10 points per a.u (max 100 but impossible in real data).(3 directions in our default Crystal
% % outputs).

% [SF,CP] = readCrystal(primVectors,'InputFiles/prop_ter.d3','InputFiles/prop_output_ter.out','InputFiles/PROF_ter.DAT',0,8, 6, 10); 

% % Same example at 100K 
...
% You can also add some noise with a given amplitude, here 3% by default:
...
% % to SFs. As in the example, 7% of DCPs point is correct. This even more
...
% % - The level of noise is usually set at 1% to get better results at 100K.
...
% %%%%%%%%% Info in the Command Window during the refinement :
...
% % CHi^2/ndf)
% % Note that : 
% % - the more data is big, the more it takes time, especially for the creation of predictors.
% % - for a reasonable choice of data, the solver only needs a few seconds to conclude.

% plotALL(solP,atomicOrbitals,R,T,SF,CP,volume,'InputFiles/DENSPatoNew.dat'); %desorthogonalization of P is included here
...
% % Be careful with the desorthogonalization of the population matrix !
...
% [SF,CP] = addNoise(SF,CP,0.01); % 1%-noisy
% [solP,C] = optimizeP_quadratic(atomicOrbitals,R,T,SF,CP);
atomicOrbitals_final = atomic_least_squares(atomicOrbitals,solP,R,T,SF,CP); % or use valence_least_squares% à améliorer et fusionner
...
%% Your turn to combine ... Other possibilities with softwares in 'Explorations' or see article.
